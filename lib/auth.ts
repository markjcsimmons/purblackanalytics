...
// If not set, defaults to 'Mark32246!' 
...
export function authenticate(password: string): AuthSession | null {
  // Hardcode the password since NEXT_PUBLIC_ vars are build-time only
  // This will work even if env vars aren't set during build
  const fullAccessPassword = process.env.NEXT_PUBLIC_FULL_ACCESS_PASSWORD || 'Mark32246!';
  
  // Normalize passwords for comparison (trim whitespace)
  const normalizedInput = password.trim();
  const normalizedFull = fullAccessPassword.trim();
  ...
  } else if (normalizedInput === PASSWORDS.LIMITED_ACCESS.trim()) {
    return {
      accessLevel: 'limited',
      isAuthenticated: true,
    };
